...
Stellar Deepdive integrates [SEP-24](https://developers.stellar.org/docs/platforms/anchor-platform/sep-guide/sep24) so users can deposit and withdraw fiat (and assets) through anchors directly from the platform, with a single UI for anchor interactions.
...
- Run: `cargo test -p stellar-deepdive-backend sep24_proxy`.

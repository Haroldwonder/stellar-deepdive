...
**Primary:** AWS S3 `s3://stellar-deepdive-backups/`
```
stellar-deepdive-backups/
├── postgres/
│   ├── full/
│   └── wal/
├── redis/
...
```bash
# /etc/environment
export WALG_S3_PREFIX=s3://stellar-deepdive-backups/postgres
...
tar -czf $BACKUP_DIR/app-configs-$DATE.tar.gz \
  /opt/stellar-deepdive/backend/.env \
  /opt/stellar-deepdive/frontend/.env.local \
  /etc/nginx/sites-available/stellar-deepdive
...
# Upload to S3
aws s3 cp $BACKUP_DIR/ s3://stellar-deepdive-backups/configs/ --recursive
...
```bash
# Download from S3
aws s3 cp s3://stellar-deepdive-backups/configs/app-configs-latest.tar.gz /tmp/

# Extract
tar -xzf /tmp/app-configs-latest.tar.gz -C /

# Restart services
sudo systemctl restart stellar-deepdive-backend
...
# Upload to S3
aws s3 cp /var/backups/redis/dump-$(date +%Y%m%d).rdb s3://stellar-deepdive-backups/redis/
```
...
aws s3 ls s3://stellar-deepdive-backups/ > /dev/null 2>&1
if [ $? -ne 0 ]; then
...
# Verify data
docker exec postgres-restore-test psql -U postgres -d stellar_deepdive -c "
...
# Clean S3 backups
aws s3 ls s3://stellar-deepdive-backups/postgres/full/ | \
  awk '{print $4}' | \
  while read backup; do
    AGE=$(( $(date +%s) - $(date -d "$(echo $backup | cut -d_ -f2)" +%s) ))
    if [ $AGE -gt 2592000 ]; then  # 30 days
      aws s3 rm s3://stellar-deepdive-backups/postgres/full/$backup
    fi
  done
```
...
aws s3 ls s3://stellar-deepdive-backups/
...
**Last Tested:** [Date]  
**Next Review:** [Date]  
**Contact:** devops@stellar-deepdive.com

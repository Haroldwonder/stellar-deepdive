/**
 * WebSocket client for real-time updates from the Stellar Deepdive backend
 */
...
export class stellarDeepdiveWebSocket {
...
let wsInstance: stellarDeepdiveWebSocket | null = null;
...
): stellarDeepdiveWebSocket {
  if (!wsInstance) {
    wsInstance = new stellarDeepdiveWebSocket(config);
  }
  return wsInstance;
}

...
use chrono::Utc;
use sqlx::SqlitePool;
use stellar_deepdive_backend::models::asset_verification::{
    ReportAssetRequest, ReportType, VerificationStatus,
};
use stellar_deepdive_backend::services::asset_verifier::AssetVerifier;
use uuid::Uuid;
...
async fn test_reputation_score_calculation() -> Result<()> {
    ...
    let result = stellar_deepdive_backend::services::asset_verifier::VerificationResult {
...
async fn test_save_and_retrieve_verification() -> Result<()> {
    ...
    let result = stellar_deepdive_backend::services::asset_verifier::VerificationResult {
        ...
        stellar_toml_data: Some(
            stellar_deepdive_backend::services::asset_verifier::StellarTomlData {
...
async fn test_unique_constraint() -> Result<()> {
    ...
    let updated_result = stellar_deepdive_backend::services::asset_verifier::VerificationResult {
        stellar_expert_verified: true,
        stellar_toml_verified: true,
        stellar_toml_data: None,
        anchor_registry_verified: false,
    ...
    let handles: Vec<_> = (0..5)
        .map(|i| {
            ...
            tokio::spawn(async move {
                ...
                let result =
                    stellar_deepdive_backend::services::asset_verifier::VerificationResult {
                        stellar_expert_verified: true,
                        stellar_toml_verified: false,
                        stellar_toml_data: None,
                        anchor_registry_verified: false,

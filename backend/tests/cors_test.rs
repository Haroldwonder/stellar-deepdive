...
async fn test_cors_production_origin_receives_acao_header() {
    let cors =
        cors_layer_from_origins("https://stellar-deepdive.com,https://www.stellar-deepdive.com");
    let app = build_router_with_cors(cors);
    ...
    let response = app
        .oneshot(
            Request::builder()
                .method(Method::GET)
                .uri("/health")
                .header(header::ORIGIN, "https://stellar-deepdive.com")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(acao, "https://stellar-deepdive.com");
}

...
use stellar_deepdive_backend::alerts::AlertManager;
use stellar_deepdive_backend::api::account_merges;
use stellar_deepdive_backend::api::anchors_cached::get_anchors;
use stellar_deepdive_backend::api::api_analytics;
use stellar_deepdive_backend::api::api_keys;
use stellar_deepdive_backend::api::asset_verification;
use stellar_deepdive_backend::api::cache_stats;
use stellar_deepdive_backend::api::corridors_cached::{get_corridor_detail, list_corridors};
use stellar_deepdive_backend::api::cost_calculator;
use stellar_deepdive_backend::api::fee_bump;
use stellar_deepdive_backend::api::liquidity_pools;
use stellar_deepdive_backend::api::metrics_cached;
use stellar_deepdive_backend::api::oauth;
use stellar_deepdive_backend::api::verification_rewards;
use stellar_deepdive_backend::api::webhooks;
use stellar_deepdive_backend::auth::AuthService;
use stellar_deepdive_backend::auth_middleware::auth_middleware;
use stellar_deepdive_backend::cache::{CacheConfig, CacheManager};
use stellar_deepdive_backend::cache_invalidation::CacheInvalidationService;
use stellar_deepdive_backend::database::Database;
// use stellar_deepdive_backend::graphql::{build_schema, AppSchema};
// use stellar_deepdive_backend::gdpr::{GdprService, handlers as gdpr_handlers};
use stellar_deepdive_backend::handlers::*;
use stellar_deepdive_backend::ingestion::ledger::LedgerIngestionService;
use stellar_deepdive_backend::ingestion::DataIngestionService;
use stellar_deepdive_backend::ip_whitelist_middleware::{
    ip_whitelist_middleware, IpWhitelistConfig,
};
use stellar_deepdive_backend::jobs::JobScheduler;
use stellar_deepdive_backend::monitor::CorridorMonitor;
use stellar_deepdive_backend::network::NetworkConfig;
use stellar_deepdive_backend::observability::{metrics as obs_metrics, tracing as obs_tracing};
use stellar_deepdive_backend::openapi::ApiDoc;
use stellar_deepdive_backend::rate_limit::{rate_limit_middleware, ClientRateLimits, RateLimitConfig, RateLimiter};
use stellar_deepdive_backend::request_id::request_id_middleware;
use stellar_deepdive_backend::rpc::StellarRpcClient;
use stellar_deepdive_backend::rpc_handlers;
use stellar_deepdive_backend::services::account_merge_detector::AccountMergeDetector;
use stellar_deepdive_backend::services::fee_bump_tracker::FeeBumpTrackerService;
use stellar_deepdive_backend::services::liquidity_pool_analyzer::LiquidityPoolAnalyzer;
use stellar_deepdive_backend::services::price_feed::{
    default_asset_mapping, PriceFeedClient, PriceFeedConfig,
};
use stellar_deepdive_backend::services::realtime_broadcaster::RealtimeBroadcaster;
use stellar_deepdive_backend::services::trustline_analyzer::TrustlineAnalyzer;
use stellar_deepdive_backend::services::webhook_dispatcher::WebhookDispatcher;
use stellar_deepdive_backend::shutdown::{
...
use stellar_deepdive_backend::state::AppState;
use stellar_deepdive_backend::telegram;
use stellar_deepdive_backend::vault;
use stellar_deepdive_backend::websocket::WsState;
...
async fn main() -> Result<()> {
    ...
    obs_tracing::init_tracing("stellar-deepdive-backend")?;
    obs_metrics::init_metrics();

    tracing::info!("Starting Stellar Deepdive Backend");

    // Validate environment configuration
    stellar_deepdive_backend::env_config::validate_env()
        .context("Environment configuration validation failed")?;

    // Log sanitized environment configuration
    stellar_deepdive_backend::env_config::log_env_config();
    ...
    let database_url = std::env::var("DATABASE_URL")
        .unwrap_or_else(|_| "sqlite:./stellar_deepdive.db".to_string());
    ...
    let pool_config = stellar_deepdive_backend::database::PoolConfig::from_env();
    tracing::info!(
    ...
    let sep10_service = Arc::new(
        stellar_deepdive_backend::auth::sep10_simple::Sep10Service::new(
            ...
            std::env::var("SEP10_HOME_DOMAIN")
                .unwrap_or_else(|_| "stellar-deepdive.local".to_string()),
    ...
    let verification_rewards_service = Arc::new(
        stellar_deepdive_backend::services::verification_rewards::VerificationRewardsService::new(
            Arc::clone(&db),
        ),
    );
    ...
    let governance_service = Arc::new(
        stellar_deepdive_backend::services::governance::GovernanceService::new(Arc::clone(&db)),
    );
    ...
    */
    ...
    let (alert_manager_raw, alert_rx) = stellar_deepdive_backend::alerts::AlertManager::new();
    let alert_manager = Arc::new(alert_manager_raw);
    tracing::info!("Alert manager initialized");

    // Initialize Corridor Monitor
    let corridor_monitor = Arc::new(stellar_deepdive_backend::monitor::CorridorMonitor::new(
        Arc::clone(&alert_manager),
        Arc::clone(&cache),
        Arc::clone(&rpc_client),
    ));
    ...
    if let Some(url) = slack_webhook_url {
        let slack_bot = stellar_deepdive_backend::services::slack_bot::SlackBotService::new(
            url,
    ...
    // Production example: CORS_ALLOWED_ORIGINS=https://stellar-deepdive.com
    ...
    // Build auth router
    let auth_routes = stellar_deepdive_backend::api::auth::routes(auth_service.clone());
    ...
    let price_routes = Router::new()
        .nest(
            "/api/prices",
            stellar_deepdive_backend::api::price_feed::routes(Arc::clone(&price_feed)),
        )
        .layer(ServiceBuilder::new().layer(middleware::from_fn_with_state(
            rate_limiter.clone(),
            rate_limit_middleware,
    ...
    let network_routes = Router::new()
        .nest(
            "/api/network",
            stellar_deepdive_backend::api::network::routes(),
        )
        .layer(ServiceBuilder::new().layer(middleware::from_fn_with_state(
            rate_limiter.clone(),
            rate_limit_middleware,
    ...
    let trustline_routes = Router::new()
        .nest(
            "/api/trustlines",
            stellar_deepdive_backend::api::trustlines::routes(Arc::clone(&trustline_analyzer)),
        )
        .layer(ServiceBuilder::new().layer(middleware::from_fn_with_state(
            rate_limiter.clone(),
            rate_limit_middleware,
    ...
    let achievements_routes = Router::new()
        .nest(
            "/api",
            stellar_deepdive_backend::api::achievements::routes(),
        )
        .layer(ServiceBuilder::new().layer(middleware::from_fn_with_state(
            rate_limiter.clone(),
            rate_limit_middleware,
    ...
    let governance_routes = Router::new()
        .nest(
            "/api/governance",
            stellar_deepdive_backend::api::governance::routes(
                governance_service,
    ...
    */
    ...
    let ws_routes = Router::new()
        .route("/ws", get(stellar_deepdive_backend::websocket::ws_handler))
    ...
    let alert_ws_routes = Router::new()
        .route(
            "/ws/alerts",
            get(stellar_deepdive_backend::alert_handlers::alert_websocket_handler),
        )
    ...
    let app = Router::new()
        ...
        .layer(middleware::from_fn_with_state(
            db.clone(),
            stellar_deepdive_backend::api_analytics_middleware::api_analytics_middleware,
        ))

...
2. Set up your `.env` file with `DATABASE_URL`:
   ```bash
   cp .env.example .env
   # Edit .env and set DATABASE_URL=sqlite:./stellar_deepdive.db
   ```

### Execute Migration

```bash
cd stellar-deepdive/backend
sqlx migrate run
```
...
sqlite3 stellar_deepdive.db ".indexes payments"

# Check all indexes
sqlite3 stellar_deepdive.db "SELECT name, tbl_name FROM sqlite_master WHERE type='index' ORDER BY tbl_name, name;"

# Verify index usage in query plan
sqlite3 stellar_deepdive.db "EXPLAIN QUERY PLAN SELECT * FROM payments WHERE created_at > datetime('now', '-1 day');"
```
...
sqlite3 stellar_deepdive.db "SELECT * FROM sqlite_stat1;"

# Analyze query performance
sqlite3 stellar_deepdive.db "EXPLAIN QUERY PLAN <your_query>;"

# Check table sizes
sqlite3 stellar_deepdive.db "
SELECT 
    name,
